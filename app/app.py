    ...
    def predict_future(self, data, days=30):
        ...
        try:
            if self.model is None or not self.is_trained:
                print(" Model not trained for prediction")
                return None, None
            ...
            last_data = df[feature_columns].iloc[-1:].copy()
            ...
            last_date = data.index[-1] if hasattr(data.index[-1], 'date') else pd.to_datetime(data.index[-1])
            
            for i in range(days):
                # Make prediction
                next_price = self.model.predict(current_data)[0]
                ...
                # For the next prediction, update features
                if i < days - 1:
                    new_row = current_data.copy()
                    # Update the close price in the new row
                    # This is a simplified update - in practice you'd update all rolling features
                    for col in new_row.columns:
                        if 'Close_Lag' in col:
                            # Shift the lagged prices
                            lag_num = int(col.split('_')[-1])
                            if lag_num == 1:
                                new_row[col] = next_price
                            else:
                                new_row[col] = current_data[f'Close_Lag_{lag_num-1}'].iloc[0]
...
def get_stock_data_historical():
    """Fetch historical Coca-Cola stock data using the working method"""
    try:
        print(" Fetching historical Coca-Cola data (2015-2024)...")
        
        # Use the method that worked for you
        ticker = 'KO'
...
def get_stock_data(ticker='KO', period='2y', max_retries=2):
    """Fetch and process stock data with multiple fallback options"""
    # First try the historical method that we know works
    historical_data = get_stock_data_historical()
    if historical_data is not None and len(historical_data) > 100:
        return historical_data
    
    # If historical method fails, try alternative approaches
...
def api_dashboard():
    ...
    # Get predictions only if model is trained
    ...
    if predictor.is_trained:
        ...
        if future_dates and future_prices:
            ...
            print(" Could not generate predictions for dashboard")
    else:
        print(" Model not trained yet - no predictions for dashboard")
